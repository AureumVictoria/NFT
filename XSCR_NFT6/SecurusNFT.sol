...
import "./IERC20.sol";
     ...
     */
    function addLicence(address paymentCoin, uint256 price) external onlyAuthorized {
     ...
     */
    function removeLicence(uint256 index) external onlyAuthorized {
     ...
     */
    function setNewReceiverAddress(address NewReceiverAddress) public onlyAuthorized {
    ...
    function updateLicence(uint tokenId, uint256 licenceIndex) external {
        licence memory l = licenses[licenceIndex];
            require(isWhitelisted(msg.sender) != false || theWhitelist != true, "Not Whitelisted");
     ...
     * msg.sender is token holder to renew the licence
     */
    function updateLicenceAdmin(uint tokenId, uint256 licenceIndex) external onlyAuthorized{
        licence memory l = licenses[licenceIndex];
            require(isWhitelisted(msg.sender) != false || theWhitelist != true, "Not Whitelisted");
            require(isBlacklisted(msg.sender) != true, "isBlacklisted");
            NftLicenceUpdateTicker += 1;
